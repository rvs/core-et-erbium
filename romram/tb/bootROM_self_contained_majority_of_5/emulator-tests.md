...
| `0x7FFFD028` | `0x8000000000000000`| OTP_CFGR copy 0 (pmode=1) |
| `0x7FFFD030` | `0x8000000000000000`| OTP_CFGR copy 1      |
| `0x7FFFD038` | `0x8000000000000000`| OTP_CFGR copy 2      |
| `0x7FFFD040` | `0x8000000000000000`| OTP_CFGR copy 3      |
| `0x7FFFD048` | `0x8000000000000000`| OTP_CFGR copy 4      |
| `0x40000020` | `0x40FFFFF0`       | MRAM_PAYL_SP         |
| `0x40000028` | `0x40000200`       | MRAM_PAYL_PC (Zephyr)|

**Mailbox0:** `0xFEF9F`
**Path:** MRAM init → no early jump → enter fwup, PC=0 skip → trim(0) → dividers(0) → jump to Zephyr
**PASS signal:** Yes
...
**Mailbox0:** `0xFC79F`
**Path:** MRAM init → no early jump → no fwup → trim(0) → dividers(0) → jump to Zephyr
**PASS signal:** Yes
...
Bootrom ELF: `ErbiumROM_test1.elf` (COSIM). Payload ELF: `zephyr.elf`.

3 copies have pmode=0 (corrupted). Voter resolves pmode=0: 2 vs 3 → corruption wins. Bootrom skips payload and enters deep sleep instead of jumping to Zephyr.
...
T7-T8 have the same mailbox as T4. T9-T10 have the same mailbox as T3.
...
| T9   | `0x001F9F`   | Yes         |
| T10  | `0x001F9F`   | Yes         |
| T11  | `0x17C79F`   | Yes (deep sleep, no payload) |
