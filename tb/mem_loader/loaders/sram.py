...
class SRAMLoader(Loader):
    ...
    def __init__(self, dut, base_addr, size):
        ...
        self.MAX_SIZE = size
        self.dut = dut
        assert size == 4*1024, "Current implementation only supports 4KB, found size {size}"

        self.sram = {
            row_idx: self.dut.ram.ram.ram[row_idx]
    ...
    async def read_byte(self, addr: int):
        ...
        msb = (row_offset*8) + 7
        lsb = row_offset*8
        return int(self.sram[row].value[msb:lsb])

    async def zero_all_mem(self):
        zero = bytes(1)
        for addr in range(0, self.MAX_SIZE, 64):
            await self.write_row(addr, zero)
