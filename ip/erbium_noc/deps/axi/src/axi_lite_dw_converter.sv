...
module erbium_noc_axi_lite_dw_converter #(
  ...
  if (AxiSlvPortDataWidth > AxiMstPortDataWidth) begin : gen_downsizer
    ...
    erbium_noc_axi_pkg::resp_t b_resp_q,    b_resp_d;
    logic           b_resp_load;
    ...
    assign b_resp_d    = (&b_sel_q) ? erbium_noc_axi_pkg::RESP_OKAY : (b_resp_q | mst_res_i.b.resp);
    assign b_resp_load = mst_res_i.b_valid & mst_req_o.b_ready;
    `FFLARN(b_sel_q, b_sel_d, b_resp_load, '0, clk_i, rst_ni)
    `FFLARN(b_resp_q, b_resp_d, b_resp_load, erbium_noc_axi_pkg::RESP_OKAY, clk_i, rst_ni)
...
module erbium_noc_axi_lite_dw_converter_intf #(
  /// AXI4-Lite address width of the ports.
  ...
  erbium_noc_AXI_LITE.Master mst
);
  // AXI configuration
  ...
  erbium_noc_axi_lite_dw_converter #(
