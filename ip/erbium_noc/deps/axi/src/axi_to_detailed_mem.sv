...
/// utilization of 50%.
module erbium_noc_axi_to_detailed_mem #(
  ...
  output erbium_noc_axi_pkg::atop_t   [NumBanks-1:0]  mem_atop_o,
  ...
  output erbium_noc_axi_pkg::cache_t  [NumBanks-1:0]  mem_cache_o,
  /// Memory stream master, protection signal.
  output erbium_noc_axi_pkg::prot_t   [NumBanks-1:0]  mem_prot_o,
  /// Memory stream master, QOS signal.
  output erbium_noc_axi_pkg::qos_t    [NumBanks-1:0]  mem_qos_o,
  /// Memory stream master, region signal.
  output erbium_noc_axi_pkg::region_t [NumBanks-1:0]  mem_region_o,
  ...
  typedef struct packed {
    addr_t            addr;
    erbium_noc_axi_pkg::atop_t   atop;
    logic             lock;
    axi_strb_t        strb;
    axi_data_t        wdata;
    logic             we;
    mem_id_t          id;
    mem_user_t        user;
    erbium_noc_axi_pkg::cache_t  cache;
    erbium_noc_axi_pkg::prot_t   prot;
    erbium_noc_axi_pkg::qos_t    qos;
    erbium_noc_axi_pkg::region_t region;
  } mem_req_t;
  ...
  typedef struct packed {
    ...
    logic             last;
    erbium_noc_axi_pkg::qos_t    qos;
    erbium_noc_axi_pkg::size_t   size;
    logic             write;
    mem_user_t        user;
    erbium_noc_axi_pkg::cache_t  cache;
    erbium_noc_axi_pkg::prot_t   prot;
  ...
  mem_rsp_t       m2s_resp;
  erbium_noc_axi_pkg::len_t  r_cnt_d,        r_cnt_q,
                  w_cnt_d,        w_cnt_q;
  ...
  always_comb begin
    ...
    if (r_cnt_q > '0) begin
      ...
      rd_meta        = rd_meta_d;
      rd_meta.addr   = rd_meta_q.addr + erbium_noc_axi_pkg::num_bytes(rd_meta_q.size);
      rd_valid       = 1'b1;
      if (rd_ready) begin
        r_cnt_d--;
    ...
    // Handle new AR if there is one.
      ...
      rd_meta_d = '{
        addr:   addr_t'(erbium_noc_axi_pkg::aligned_addr(erbium_noc_axi_pkg::largest_addr_t'(axi_req_i.ar.addr), axi_req_i.ar.size)),
        atop:   '0,
  ...
  always_comb begin
    ...
    if (w_cnt_q > '0) begin
      ...
      wr_meta        = wr_meta_d;
      wr_meta.addr   = wr_meta_q.addr + erbium_noc_axi_pkg::num_bytes(wr_meta_q.size);
    ...
    // Handle new AW if there is one.
      ...
      wr_meta_d = '{
        addr:   addr_t'(erbium_noc_axi_pkg::aligned_addr(erbium_noc_axi_pkg::largest_addr_t'(axi_req_i.aw.addr), axi_req_i.aw.size)),
  ...
  erbium_noc_mem_stream_to_banks_detailed #(
    .AddrWidth   ( AddrWidth         ),
    .DataWidth   ( DataWidth         ),
    .RUserWidth  ( 2                 ),
    .NumBanks    ( NumBanks          ),
  ...
  erbium_noc_stream_fork_dynamic #(
    .N_OUP ( 32'd2 )
  ) i_fork_dynamic (
    .clk_i,
    .rst_ni,
  ...
  assign axi_resp_o.b = '{
    id:   meta_buf.id,
    resp: next_collect_b_err ? erbium_noc_axi_pkg::RESP_SLVERR : next_collect_b_exokay ? erbium_noc_axi_pkg::RESP_EXOKAY : erbium_noc_axi_pkg::RESP_OKAY,
    user: '0
  };
  ...
  assign axi_resp_o.r = '{
    ...
    id:   meta_buf.id,
    last: meta_buf.last,
    resp: resp_r_err ? erbium_noc_axi_pkg::RESP_SLVERR : resp_r_exokay ? erbium_noc_axi_pkg::RESP_EXOKAY : erbium_noc_axi_pkg::RESP_OKAY,
    user: '0
  };
  ...
  assert property (@(posedge clk_i) axi_req_i.ar_valid && axi_req_i.ar.len > 0 |->
      axi_req_i.ar.burst == erbium_noc_axi_pkg::BURST_INCR)
    else $error("Non-incrementing bursts are not supported!");
  assert property (@(posedge clk_i) axi_req_i.aw_valid && axi_req_i.aw.len > 0 |->
      axi_req_i.aw.burst == erbium_noc_axi_pkg::BURST_INCR)
    else $error("Non-incrementing bursts are not supported!");
...
module erbium_noc_axi_to_detailed_mem_intf #(
  ...
  output erbium_noc_axi_pkg::atop_t   [NUM_BANKS-1:0]  mem_atop_o,
  ...
  output erbium_noc_axi_pkg::cache_t  [NUM_BANKS-1:0]  mem_cache_o,
  /// See `axi_to_detailed_mem`, port `mem_prot_o`.
  output erbium_noc_axi_pkg::prot_t   [NUM_BANKS-1:0]  mem_prot_o,
  /// See `axi_to_detailed_mem`, port `mem_qos_o`.
  output erbium_noc_axi_pkg::qos_t    [NUM_BANKS-1:0]  mem_qos_o,
  /// See `axi_to_detailed_mem`, port `mem_region_o`.
  output erbium_noc_axi_pkg::region_t [NUM_BANKS-1:0]  mem_region_o,
  ...
  erbium_noc_axi_to_detailed_mem #(
    .axi_req_t    ( req_t          ),
    .axi_resp_t   ( resp_t         ),
...
module erbium_noc_mem_stream_to_banks_detailed #(
  /// Input address width.
