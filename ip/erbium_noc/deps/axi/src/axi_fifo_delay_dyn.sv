...
module erbium_noc_axi_fifo_delay_dyn #(
  // AXI channel types
  ...
  if (DepthAR > 0) begin
    erbium_noc_stream_fifo_delay_dyn #(
      .payload_t ( ar_chan_t ),
      .MaxDelay  ( MaxDelay  ),
      .Depth     ( DepthAR   )
    ) i_ar_fifo_delay (
...
module erbium_noc_axi_fifo_delay_dyn_intf #(
  ...
  erbium_noc_AXI_BUS.Slave                slv,        // Slave port
  erbium_noc_AXI_BUS.Master               mst         // Master port
);
...
module erbium_noc_stream_fifo_delay_dyn #(
