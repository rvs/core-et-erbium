...
module erbium_noc_axi_lite_to_apb #(
  ...
  typedef struct packed {
    addr_t          addr;
    erbium_noc_axi_pkg::prot_t prot; // prot has the exact same bit mapping in AXI4-Lite as In APB v2.0
    data_t          data;
    strb_t          strb;
    logic           write;
  ...
  typedef struct packed {
    ...
    erbium_noc_axi_pkg::resp_t resp; // response bit from APB
  } int_resp_t; // internal
  typedef enum logic {
  ...
  erbium_noc_axi_pkg::resp_t axi_bresp;
  ...
  erbium_noc_axi_pkg::resp_t apb_wresp;
  ...
  if (PipelineRequest) begin : gen_req_spill
    ...
    erbium_noc_fall_through_register #(
      .T  ( int_req_t )
    ) i_req_ft_reg (
      .clk_i,
      .rst_ni,
  ...
  always_comb begin
    ...
    apb_wresp       = erbium_noc_axi_pkg::RESP_SLVERR;
    apb_wresp_valid = 1'b0;
    apb_rresp       = '{data: data_t'(32'hDEA110C8), resp: erbium_noc_axi_pkg::RESP_SLVERR};
        ...
        if (apb_req_valid && apb_wresp_ready && apb_rresp_ready) begin
          if (apb_dec_valid && ((apb_req.write && (|apb_req.strb)) || (!apb_req.write))) begin
            ...
            apb_req_o[apb_sel_idx] = '{
              ...
              paddr:   erbium_noc_axi_pkg::aligned_addr(erbium_noc_axi_pkg::largest_addr_t'(apb_req.addr), $clog2(DataWidth/8)),
              pprot:   apb_req.prot,
              psel:    1'b1,
              penable: 1'b0,
            ...
            if (apb_req.write) begin
              apb_wresp       = ~(|apb_req.strb) ? erbium_noc_axi_pkg::RESP_OKAY : erbium_noc_axi_pkg::RESP_DECERR;
              apb_wresp_valid = 1'b1;
            end else begin
              apb_rresp.resp  = erbium_noc_axi_pkg::RESP_DECERR;
        ...
        if (apb_resp_i[apb_sel_idx].pready) begin
          ...
          if (apb_req.write) begin
            apb_wresp       = apb_resp_i[apb_sel_idx].pslverr ?
                                  erbium_noc_axi_pkg::RESP_SLVERR : erbium_noc_axi_pkg::RESP_OKAY;
            apb_wresp_valid = 1'b1;
          end else begin
...
module erbium_noc_axi_lite_to_apb_intf #(
  parameter int unsigned NoApbSlaves = 32'd1,  // Number of connected APB slaves
  parameter int unsigned NoRules     = 32'd1,  // Number of APB address rules
  parameter int unsigned AddrWidth   = 32'd32, // Address width
  parameter int unsigned DataWidth   = 32'd32, // Data width
  ...
  typedef struct packed {
    addr_t          paddr;   // same as AXI4-Lite
    erbium_noc_axi_pkg::prot_t pprot;   // same as AXI4-Lite, specification is the same
