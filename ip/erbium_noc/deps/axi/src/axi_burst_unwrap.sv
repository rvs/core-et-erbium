...
module erbium_noc_axi_burst_unwrap #(
  ...
  function bit txn_supported(erbium_noc_axi_pkg::atop_t atop, erbium_noc_axi_pkg::burst_t burst, erbium_noc_axi_pkg::cache_t cache,
    ...
    if (!erbium_noc_axi_pkg::modifiable(cache) && (burst == erbium_noc_axi_pkg::BURST_WRAP)) begin
      return 1'b0;
    end
  ...
  erbium_noc_axi_pkg::len_t  b_cnt_len, w_cnt_len;
  erbium_noc_axi_burst_unwrap_ax_chan #(
    .AwChan   ( 1'b1         ),
    .chan_t   ( aw_chan_t    ),
    .AddrWidth ( AddrWidth   ),
    .IdWidth  ( IdWidth      ),
  ...
  erbium_noc_axi_pkg::len_t  r_cnt_len, unc1;
  erbium_noc_axi_burst_unwrap_ax_chan #(
    .AwChan    ( 0           ),
    .chan_t    ( ar_chan_t   ),
    .AddrWidth ( AddrWidth   ),
  ...
  assume property (@(posedge clk_i) slv_req_i.aw_valid |->
      slv_req_i.aw.atop == '0 || slv_req_i.aw.atop[5:4] == erbium_noc_axi_pkg::ATOP_ATOMICSTORE
...
module erbium_noc_axi_burst_unwrap_ax_chan #(
  ...
  input  id_t                 cnt_id_i,
  output erbium_noc_axi_pkg::len_t [1:0] cnt_len_o,
  ...
  erbium_noc_axi_pkg::len_t split_len, alloc_len_0;

  // Count down from upstream burst len to generate upstream last flag.
  erbium_noc_axi_burst_counters #(
    .MaxTxns ( MaxTxns  ),
    .IdWidth ( IdWidth  )
    ...
    unique case (state_q)
      Idle: begin
        if (ax_valid_i && &cnt_alloc_gnt) begin
          if (ax_i.burst == erbium_noc_axi_pkg::BURST_WRAP && ax_i.addr != wrap_boundary) begin // Splitting required.
            // Store Ax, allocate a counter, and acknowledge upstream.
            ax_d          = ax_i;
            ax_d.burst    = erbium_noc_axi_pkg::BURST_INCR;
            split_len     = 8'd1;
            ...
            if (ax_i.burst == erbium_noc_axi_pkg::BURST_WRAP) begin
              ax_o.burst = erbium_noc_axi_pkg::BURST_INCR;
            end
            ax_valid_o = 1'b1;
...
module erbium_noc_axi_burst_counters #(
  parameter int unsigned MaxTxns = 0,
  parameter int unsigned IdWidth = 0,
  parameter type         id_t    = logic [IdWidth-1:0]
) (
  ...
  typedef logic [$bits(erbium_noc_axi_pkg::len_t):0] cnt_t;
