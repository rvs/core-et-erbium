...
module erbium_noc_boxcar #(
    parameter int unsigned Width = 32,
    /// Derived parameter *Do not override*
    localparam int unsigned IdxWidth = erbium_noc_cf_math_pkg::idx_width(Width),
    ...
    erbium_noc_heaviside #(.Width(Width)) i_lo (.x_i(lsb_i), .mask_o(low_mask));
    erbium_noc_heaviside #(.Width(Width)) i_hi (.x_i(msb_i), .mask_o(high_mask_n));
