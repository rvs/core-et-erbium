...
module erbium_noc_clk_mux_glitch_free #(
  ...
  for (genvar i = 0; i < NUM_INPUTS; i++) begin : gen_input_stages
    ...
    erbium_noc_sync #(.STAGES(NUM_SYNC_STAGES)) i_sync_en(
      .clk_i    ( clks_i[i]                       ),
...
module erbium_noc_clk_or_tree #(
  ...
  if (NUM_INPUTS < 1) begin : gen_error
    ...
    erbium_noc_tc_clk_or2 i_clk_or2 (
      .clk0_i(clks_i[0]),
      .clk1_i(clks_i[1]),
      .clk_o
    );
    ...
    erbium_noc_clk_or_tree #(NUM_INPUTS/2) i_or_branch_a (
      .clks_i(clks_i[0+:NUM_INPUTS/2]),
      .clk_o(branch_a)
    );

    erbium_noc_clk_or_tree #(NUM_INPUTS/2 + NUM_INPUTS%2) i_or_branch_b (
