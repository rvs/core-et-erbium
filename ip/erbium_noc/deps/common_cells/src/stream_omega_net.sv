...
module erbium_noc_stream_omega_net #(
  ...
  if (NumInp <= Radix && NumOut <= Radix) begin : gen_degenerate_omega_net
    ...
    localparam int unsigned NumLanes = (NumOut > NumInp) ?
        unsigned'(Radix**(erbium_noc_cf_math_pkg::ceil_div($clog2(NumOut), $clog2(Radix)))) :
        unsigned'(Radix**(erbium_noc_cf_math_pkg::ceil_div($clog2(NumInp), $clog2(Radix))));
