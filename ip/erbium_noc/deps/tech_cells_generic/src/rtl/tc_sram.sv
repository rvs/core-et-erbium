...
module erbium_noc_tc_sram #(
  ...
  // memory array
  data_t erbium_noc_sram [NumWords-1:0];
  ...
  if (Latency == 32'd0) begin : gen_no_read_lat
    for (genvar i = 0; i < NumPorts; i++) begin : gen_port
      assign rdata_o[i] = (req_i[i] && !we_i[i]) ? erbium_noc_sram[addr_i[i]] : erbium_noc_sram[r_addr_q[i]];
    end
    ...
    always_comb begin
      for (int unsigned i = 0; i < NumPorts; i++) begin
        ...
        rdata_d[i][Latency-1] = (req_i[i] && !we_i[i]) ? erbium_noc_sram[addr_i[i]] : erbium_noc_sram[r_addr_q[i]];
      end
    end
  end
          ...
          if (req_i[i]) begin
            if (we_i[i]) begin
              ...
              for (int unsigned j = 0; j < BeWidth; j++) begin
                if (be_i[i][j]) begin
                  erbium_noc_sram[addr_i[i]][j*ByteWidth+:ByteWidth] <= wdata_i[i][j*ByteWidth+:ByteWidth];
                end
              end
            end else begin
