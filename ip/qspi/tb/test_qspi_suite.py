"""
QSPI Comprehensive Unit Test Suite
====================================
Follows the same pattern as ai/test_uart_suite.py:
  - QSPIEnv (RAL-based) for register access via tb.reg.*
  - SpiFlashModel for SPI protocol-level tests
  - Numbered tests 01..25
...
async def test_01_register_reset_values(dut):
    """Verify DCR, SR read back their RDL-specified reset values.
    NOTE: silicon requires cr_en=1 for any AXI read (if_abort preempts reads
    when cr_en=0).  CR itself cannot be read at reset; enable() is called first
    and CR is skipped.  See bsv_bugs.md Bug 1.
    """
    ...
    await tb.enable()   # cr_en=1 required before any read

    # CR reset value cannot be verified: enable() sets cr_en=1 (see bsv_bugs.md Bug 1)
    cocotb.log.info("  CR  reset check skipped (silicon: reads require cr_en=1)")
...
async def test_02_cr_fields_rw(dut):
    ...
    for prescaler, sshift, dmaen, enable in cases:
        ...
        await Timer(1, 'us')
        if not enable:
            # silicon: cr_en=0 blocks all reads; write val|cr_en to re-enable while
            # preserving prescaler/sshift/dmaen (bsv_bugs.md Bug 1 — enable() alone
            # would overwrite the prescaler with just 0x00000001)
            await tb.axim.write(tb._base + QspiRegs.CR, (val | (1 << CR_EN)).to_bytes(8, "little"))
        rb = await tb.reg.CR.read()
        # mask to written fields only (ignore RO status bits); ABORT(1) excluded;
        # for enable=0 cases cr_en is forced to 1 for read-back, so exclude bit 0
        mask = 0xFF000014 if not enable else 0xFF000015
...
async def test_03_dcr_fields_rw(dut):
    ...
    tb = await _tb_init(dut)
    await tb.enable()   # cr_en=1 required before any AXI read (bsv_bugs.md Bug 1)
    cocotb.log.info("=" * 60)
