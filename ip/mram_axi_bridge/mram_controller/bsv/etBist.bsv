...
default_reset="RST_B"
*)
module mkEtBist(Bist_Ifc);
	BistEtMramTranslatorIfc translator <- mkBistEtMramTranslator;
	Wire#(Bist_Cfg_Regs) csr <- mkWire();
	Probe#(Bool)	csr_start <- mkProbe();
	...
	Reg#(Bit#(79)) mram_rdata <- mkRegA(0);
	...
	Reg#(Bit#(21)) prv_current_address <- mkRegA(0);
	...
	function Action read_word_no_queue(Bit#(20) target_addr);
		return action

			translator.read_word(target_addr);
			outgoing_reads.deq();
			outgoing_reads.enq(1);
		endaction;
	endfunction
	function Action add_read_queue(Bit#(20) target_addr);
		return action
	...
	function Action read_word(Bit#(20) target_addr);
		return action
			read_word_no_queue(target_addr);
			add_read_queue(target_addr);
		endaction;
	endfunction
	function Action read_word_ref(Bit#(20) target_addr);
		return action
			translator.read_ref_word(target_addr);
			outgoing_reads.deq();
			outgoing_reads.enq(1);
			fifo_loop_cnt.enq(loop_count);
			fifo_addr.enq(target_addr);
	...
	function Action write_word(Bit#(20) target_addr, Bit#(79) din);
		return action
			translator.write_word(target_addr, din);
		endaction;
	endfunction
	function Action consume_translator_response();
		return action
			let resp <- translator.response_blocking;
			if (resp.is_read_ack)
				mram_rdata <= resp.data;
		endaction;
	endfunction
	...
	let check_error=action
		...
		Vector#(79, Bit#(1)) m = replicate((csr.data_inv? 1 : 0) & fifo_loop_cnt.first[0]);
	...
	let count_bit_errors=action
		...
		let update_status = csr_status;
		if ((errors_in_word != 0) && (update_status.error_count[16] == 1'b0)) begin
			UInt#(17) next_count =
				zeroExtend(unpack(update_status.error_count[15:0])) +
				zeroExtend(unpack(pack(errors_in_word)));
			let next_count_bits = pack(next_count);
			if (next_count_bits[16] == 1'b1) begin
				update_status.error_count = 17'h1_0000;
				if (csr.stop_on_error) begin
					update_status.error_loop = fifo_loop_cnt.first[15:0];
					update_status.error_address = fifo_addr.first;
					update_status.bist_error = True;
				end
			end else
				update_status.error_count = next_count_bits;
		end
		csr_status <= update_status;
	endaction;

	let error_count_action=action
		if (csr.stop_on_error && !csr.stop_on_repl_of)
			check_error;
			...
			while(True) seq
				...
				if (((current_address <= {0, csr.stop_addr}) && (loop_count <= {0, csr.loop_cnt}))) seq
					action
						...
						prv_current_address <= current_address;
						if (csr.stop_on_error || csr.stop_on_repl_of) 
							add_read_queue(current_address[19:0]);
					endaction
					consume_translator_response();
					if (csr.stop_on_error || csr.stop_on_repl_of) seq
						read_word_no_queue(prv_current_address[19:0]);
						consume_translator_response();
						error_count_action;
					endseq 
				continue;
	...
	function Stmt write_ref_pattern(Bit#(79) ref_data, Bit#(20) ref_addr);
		let rs = seq
			action
				translator.write_ref_word(ref_addr, ref_data);
			endaction
			consume_translator_response();
		endseq;
		return rs;
	...
	function Stmt initialize_wordline(Bit#(79) init_din, Reg#(Bit#(21)) init_addr);
		let rs =  seq
			for (init_addr[3:0] <= 0; init_addr[3:0] < 15; init_addr[3:0] <= init_addr[3:0] + 1) seq
				translator.write_word(init_addr[19:0], init_din);
				consume_translator_response();
			endseq
			// For init_addr = 15
			translator.write_word(init_addr[19:0], init_din);
			...
			if (csr.trim_mode) seq 
				action
					Bit#(20) sibling_addr = init_addr[19:0] ^ (1 << 13);
					translator.write_ref_word(sibling_addr, init_din);
				endaction
				consume_translator_response();
			endseq
		endseq;
	...
	function Stmt check_wordline_for_error(Bit#(1) xor_factor, Reg#(Bit#(21)) init_addr);
		let rs =  seq
			...
			endaction
				while (True) seq
					// Keep response-consume and next-read launch in separate cycles to
					// avoid dynamic scheduling across translator methods in Bluesim.
					action
						if (read_cnt < 16) begin
							...
							read_cnt <= read_cnt + 1;
							read_word_ref({init_addr[19:4], read_cnt[3:0]});
						end  
					endaction
					consume_translator_response();
					action
				...
				if (!csr.trim_mode && (read_cnt == 16)) 	break;
				else if (csr.trim_mode && (read_cnt == 17)) break;
			...
			for (current_address[20:4] <= {0,csr.start_addr[19:4]}; current_address[19:4] <= csr.stop_addr[19:4]; current_address[20:4] <= current_address[20:4] + 1) seq
				...
				while (True) seq
					...
					csr_status.rh1 <= rh_set;
						...
						// rh2 = ((rh0 + rh1) >> 1) + rh2_offset
						action
							Bit#(8) rh2_midpoint = ({1'b0, csr_status.rh1} + {1'b0, csr_status.rh0}) >> 1;
							Int#(8) rh2_with_offset =
								unpack(rh2_midpoint) + signExtend(csr.rh2_offset);
							rh_set_ovrflow <= rh2_midpoint;
							csr_status.rh2 <= truncate(pack(rh2_with_offset));
	...
	interface BistEtMramPipeIfc mram = translator.mram;
